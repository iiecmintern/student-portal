...
// Compound unique index to ensure one enrollment per user per course
...
enrollmentSchema.virtual('duration').get(function() {
  ...
  return Math.floor((endDate - this.enrolled_at) / (1000 * 60 * 60 * 24)); // days
});

// Method to update progress
enrollmentSchema.methods.updateProgress = async function(lessonId) {
  if (!this.progress.completed_lessons.includes(lessonId)) {
  ...
  if (course && course.lessons.length > 0) {
    this.progress.overall_progress = Math.round(
      (this.progress.completed_lessons.length / course.lessons.length) * 100
    );
